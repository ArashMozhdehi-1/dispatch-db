    ...
    segments: async (_, { limit = 1000 }) => {
      try {
        ...
        return result.rows.map(row => ({
          ...
          lane_width_m: row.lane_width_m ? parseFloat(row.lane_width_m) : null,
          weight_limit_tonnes: row.weight_limit_tonnes ? parseFloat(row.weight_limit_tonnes) : null,
      ...
      try {
        const result = await query(`
          SELECT 
            ...
            tl.length_m,
            tl.is_active,
            tl.start_lat as start_latitude,
            tl.start_lon as start_longitude,
            tl.end_lat as end_latitude,
            tl.end_lon as end_longitude,
            ST_AsGeoJSON(ST_MakeLine(
              ST_SetSRID(ST_MakePoint(tl.start_lon, tl.start_lat), 4326),
              ST_SetSRID(ST_MakePoint(tl.end_lon, tl.end_lat), 4326)
            )) as geometry
          FROM trolley_lanes tl
          WHERE tl.is_active = true
          ORDER BY tl.road_id, tl.lane_id
        `);
        ...
        return result.rows.map(row => ({
          ...
          weight_limit_tonnes: 80, // Default trolley weight limit
          direction: row.direction,
          length_m: parseFloat(row.length_m),
          time_empty_seconds: 45.0, // Default trolley times
          time_loaded_seconds: 60.0,
          is_closed: !row.is_active,
          geometry: row.geometry, // Use the actual geometry from PostGIS
          start_latitude: parseFloat(row.start_latitude),
          start_longitude: parseFloat(row.start_longitude),
          end_latitude: parseFloat(row.end_latitude),
          end_longitude: parseFloat(row.end_longitude),
          ...
          trolley_catenary_type: 'standard',
          trolley_supports: [],
          trolley_conditions: []
        }));
      } catch (error) {
        console.error('Error fetching trolley segments:', error);
        throw new Error('Failed to fetch trolley segments');
      }
    },
