...
def populate_consolidated_locations():
    ...
    try:
        ...
        sample_locs = list(location_map.items())[:3]
        for loc_name, loc_data in sample_locs:
            print(f"   Sample: {loc_name} - {len(loc_data['coordinates'])} coordinates")
        
        # Count locations with enough points
        valid_locations = {k: v for k, v in location_map.items() if len(v['coordinates']) >= 3}
        print(f"📊 Locations with >= 3 points: {len(valid_locations)}")
        ...
        for loc_name, loc_data in location_map.items():
            ...
            try:
                ...
                # Create polygon WKT from coordinates (simpler approach)
                coord_pairs = [f"{c['lon']} {c['lat']}" for c in coords]
                # Close the polygon
                ...
                print(f"   ⚠️ Failed to process {loc_name}: {e}")
                import traceback
                traceback.print_exc()
                continue
        
        print(f"✅ Inserted {inserted} consolidated locations")
        print(f"⚠️ Skipped {skipped} locations (need >= 3 points)")
