...
def trim_roads_under_intersections(conn, trim_buffer_m: float = 0.0):
    """
    Trim lane_segments strictly to the intersection polygon (no buffer).
    NOTE: trim_buffer_m is only for logging; the SQL uses intersection polygons as-is.
    """
    try:
        cur = conn.cursor()
        cur.execute(
            ...
            trimmed AS (
              ...
              JOIN inter_union iu ON iu.geom IS NOT NULL AND ST_Intersects(ls.geometry, iu.geom)
            )
            UPDATE lane_segments ls
            SET geometry = t.new_geom
            FROM trimmed t
            WHERE t.rid = ls.ctid;
            """
        )
        conn.commit()
        print(f"✂️  Trimmed roads inside intersections (buffer={trim_buffer_m} m)")
    except Exception as e:
        print(f"❌ Error trimming roads: {e}")
        conn.rollback()
...
def main():
    ...
    try:
        ...
        # Populate intersection_center_points directly from branch_centers returned by build_intersection_polygons
        cur = conn.cursor()
        # Recreate table to ensure schema includes side
        cur.execute("DROP TABLE IF EXISTS intersection_center_points;")
        cur.execute(
            """
            CREATE TABLE intersection_center_points (
        ...
        inserted_centers = 0
        for res in results:
            centers = res.get("branch_centers") or []
            if not centers:
                continue
            for center_geo in centers:
                if not isinstance(center_geo, dict):
                    continue

                geom_json = center_geo.get("geometry")
                road_id_val = center_geo.get("road_id")
                side_val = center_geo.get("side", "unknown")

                # Must have a road_id; ignore stray/unknown
                if road_id_val is None:
                    continue

                # Ignore legacy 'center' side; keep only left/right
                if side_val == "center":
                    continue

                cur.execute(
                    """
                    INSERT INTO intersection_center_points
                      (road_id, intersection_id, intersection_name, side, geometry)
                    VALUES
                      (
                        %(road_id)s,
                        %(intersection_id)s,
                    ...
                    """,
                    {
                        ...
                        "intersection_name": res.get("intersection_name"),
                        "side": side_val,
        ...
        conn.commit()
        print(f"✅ Inserted {inserted_centers} branch centre points")

        # After intersections are written, trim road geometries strictly to intersection polygons (no buffer)
        trim_roads_under_intersections(conn, trim_buffer_m=0.0)
