...
def get_road_side_centers_at_intersection(
    ...
    if from_marker_oid and to_marker_oid:
        ...
        # Dispatch ETL path: use intersection_center_points + intersections
        query = """
    WITH centers AS (
        SELECT
            road_id,
            intersection_id,
            intersection_name,
            ST_Transform(geometry, %s) AS geom
    ...
    SELECT
        ST_X(f.geom) AS from_x,
        ST_Y(f.geom) AS from_y,
        NULL AS from_segment_wkt,
        ST_X(t.geom) AS to_x,
        ST_Y(t.geom) AS to_y,
        NULL AS to_segment_wkt,
        ST_AsText(i.geom) AS intersection_wkt,
        ST_X(ST_Centroid(i.geom)) AS intersection_cx,
        ST_Y(ST_Centroid(i.geom)) AS intersection_cy,
        ST_X(ST_ClosestPoint(ST_Boundary(i.geom), f.geom)) AS from_boundary_x,
        ST_Y(ST_ClosestPoint(ST_Boundary(i.geom), f.geom)) AS from_boundary_y,
        ST_X(ST_ClosestPoint(ST_Boundary(i.geom), t.geom)) AS to_boundary_x,
        ST_Y(ST_ClosestPoint(ST_Boundary(i.geom), t.geom)) AS to_boundary_y
    FROM from_center f, to_center t, intersection_poly i;
    """
...
def clip_centerline_and_envelope_to_roads(
    ...
    if (
        ...
        # (Snippet truncated in source view but logic implies fallback)
        return {
